...
junccov <- junccov %>%
  ...
  dplyr::mutate(score_gthr0.75 = junction_score(uniqreads, mmreads, pred.cov, 
                                                g = glist$gthr0.75, beta = 1),
                score_gsigmoid = junction_score(uniqreads, mmreads, pred.cov,
                                                g = glist$gsigmoid,beta = 1),
                score_glinear = junction_score(uniqreads, mmreads, pred.cov,
                                               g = glist$glinear, beta = 1)) %>%
  dplyr::mutate(scaled.cov_gthr0.75 = scaled_coverage(uniqreads, mmreads, pred.cov, 
                                                      g = glist$gthr0.75, beta = 1),
                scaled.cov_gsigmoid = scaled_coverage(uniqreads, mmreads, pred.cov,
                                                      g = glist$gsigmoid, beta = 1),
                scaled.cov_glinear = scaled_coverage(uniqreads, mmreads, pred.cov,
                                                     g = glist$glinear, beta = 1)) %>%
  dplyr::ungroup()
...
genecov <- dplyr::left_join(genecov, 
                            junccov %>% dplyr::select(gene, method, score_gthr0.75,
                                                      score_gsigmoid, score_glinear) %>%
                              dplyr::distinct(),
                            by = c("gene", "method"))
